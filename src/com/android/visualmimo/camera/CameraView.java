...
public class CameraView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void setImageProcessor(ImageProcessing processor) {
    	this.imageProcessor = processor;
    	
		if( imageProcessor != null && camera != null ) {
			imageProcessor.initSizeFromCamera(camera);
		}
    }
            ...
            public void onPreviewFrame(final byte[] frame, final Camera camera) {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        ...
                        Log.e("??", "Added frame to cache");
                        if(imageProcessor != null)
                        	Log.e("??", "Processor not null!!");
                        	imageProcessor.convertPreview(cache.getRecentFrame(0), cache.getRecentFrame(1), camera);
                    }
                }).start();
                camera.addCallbackBuffer(previewBuffer());
            }
